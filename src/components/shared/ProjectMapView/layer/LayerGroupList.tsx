...
type LayerGroupItemProps = {
  ...
  onDownloadGroup: (groupId: string) => void
  onMenuSelect: (item: ItvMenuItem, group: ProjectMapViewGroup) => void
  onSelectGroup: (group: ProjectMapViewGroup) => void
}

type LayerGroupHeaderProps = {
  group: ProjectMapViewGroup
  pageLevel: ProjectMapViewPageLevel
  layerVisibility: Record<string, boolean>
  selectedGroup: string | null
...
const LayerGroupHeader: React.FC<LayerGroupHeaderProps> = ({
  ...
  const handleGroupToggleClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    e.stopPropagation()
    onToggleGroupOpen(group.groupId)
  }

  const handleGroupVisibilityChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    e.stopPropagation()
...
const LayerGroupDetails: React.FC<LayerGroupDetailsProps> = ({
  group,
  layerVisibility,
    ...
    <AccordionDetails className='pt-0 pb-0 pl-2'>
      {group.layers.map((layer) => (
        <div key={layer.id} className='flex min-w-0 items-center'>
          ...
          <label htmlFor={layer.id} className='wrap-break-word ml-2 min-w-0 flex-1 py-2 text-sm'>
...
const LayerGroupItem: React.FC<LayerGroupItemProps> = ({
  ...
  return (
    <Draggable key={group.groupId} draggableId={group.groupId} index={index} isDragDisabled={!isReordering}>
      {(provided, snapshot) => (
...
const LayerGroupList: React.FC<Props> = ({
  ...
  const handleToggleGroupOpen = (groupId: string) => {
    setOpen((prev) => ({ ...prev, [groupId]: !prev[groupId] }))
  }

  const handleSelectGroup = (group: ProjectMapViewGroup) => {
